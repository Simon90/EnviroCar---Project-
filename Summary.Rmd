...
`@endtime` to store the end time when having generalised line geometries with one value per attribute for a set of points, `@data` to store the attributes and `@connections` to keep a record of attribute data between points like speed and distance.
